...
function main() {
    ...
    chokidar.watch(config.snippetDir).on("change", function (entryName) {
        ...
        console.log("SNIPPET: Updated key ".concat((0, utils_1.normalizeTemplateKey)(entryName, config.snippetDir), " (").concat(entryName, ")"));
    });
    // Watch source files
    chokidar.watch(config.watchDir).on("change", function (entryName) {
        ...
        if (entryName.includes(config.watchDir) &&
            ...
            return;
        (0, utils_1.compileTemplate)(entryName, snippets, config.snippetDir, config.watchDir, config.distDir);
    });
}
exports.main = main;
